...
- **Sort**: groups by `year` ascending. Within a group, episodes sort by `(series.topic, series.seriesNumber, series.partNumber)` first so multi-part series read in narrative order; standalone episodes (no `series`) come after, ordered by `publishedAt`.
