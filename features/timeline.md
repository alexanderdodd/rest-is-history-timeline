...
- **Sort**: groups by `year` ascending. Within a group, episodes sort by `(productionId, series.partNumber)` first so multi-part series read in narrative order; standalone episodes (no `series`) come after, ordered by `publishedAt`. `productionId` is `(topicSlug, seriesNumber, publishedAt-cluster)` — two distinct productions that both happen to be called e.g. "French Revolution Series 1" (the older 5-part series and the newer 7-part Goalhanger one) are split apart by a publish-gap heuristic in `episodes-loader.ts` so they don't fight over the same Part-N slots.
- **Series badge**: when an episode has a `series` field from the classifier, a small uppercase badge appears above the title in the form `"<topic>: Series <n> - Part <m> of <total>"` (e.g. "The French Revolution: Series 1 - Part 2 of 5"). Each (topic, seriesNumber) pair gets a deterministic colour from a hash of its id.
