...
- **Host detection** (added `2026-05-04.v8`): adds a `hostsOnly: boolean` to every classified episode, populated by the LLM from title + description signals. Powers the "hosts only" timeline filter — letting the user exclude guest episodes when they want pure Tom + Dominic. Defaults to `true` when ambiguous because the show is hosts-only by default; explicit guest signals ("with X", "joined by Y", description naming a guest) flip it to `false`. Schema-additive: existing entries without the field are treated as `hostsOnly: true` by the page.
